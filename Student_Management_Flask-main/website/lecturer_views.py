...
def lecturer_input_scores(lecturer_id, course_class_id):
	"""Trang nhập điểm cho lớp học (giữ nguyên vì có POST logic)"""
		...
		try:
			upsert_sql = """
				...
				ON DUPLICATE KEY UPDATE
				  attendance_scr = VALUES(attendance_scr),
				  midterm_scr   = VALUES(midterm_scr),
				  finalterm_scr = VALUES(finalterm_scr)
			"""
			...
			for i in range(count):
				...
				params = (sid, course_class_id, att_v, mid_v, fin_v)
				cur.execute(upsert_sql, params)
			
			conn.commit()
			flash('Lưu điểm thành công!')
